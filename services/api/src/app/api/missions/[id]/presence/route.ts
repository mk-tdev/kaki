...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  } catch { return Response.json({error:"Could not load check-ins."},{status:500}); }
}
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const identity=await getIdentity();
  if (!identity) return Response.json({error:"Unauthorized"},{status:401});
  const parsed=schema.safeParse(await request.json().catch(()=>null));
  if (!parsed.success) return Response.json({error:"Invalid check-in."},{status:400});
  const {id}=await params;
  ...
  } catch { return Response.json({error:"Only a participant in an active mission can check in."},{status:409}); }
}
