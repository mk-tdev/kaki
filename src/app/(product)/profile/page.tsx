...
export default function ProfilePage() {
  ...
  async function signOut() {
    if (profile.isGuest && !window.confirm("Signing out loses this temporary identity and access to its missions. Continue?")) return;
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <section className="paper-card mt-8 rounded-[36px] p-6 sm:p-8">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-center"><Avatar profile={profile} size="lg" /><div className="flex-1"><div className="flex flex-wrap items-center gap-2"><h2 className="text-2xl font-black">{profile.name}</h2><span className="inline-flex items-center gap-1 rounded-full bg-mint px-2.5 py-1 text-[11px] font-black text-[#17654d]"><ShieldCheck className="size-3" />{profile.verified ? "Verified" : profile.isGuest ? "Guest · not verified" : "Not identity-verified"}</span></div><p className="mt-1 text-muted">{roleLabels[profile.role]} · Pek Kio community</p><p className="mt-3 max-w-xl text-sm leading-6 text-muted">{profile.bio || "Ready to share small moments of help with the neighbourhood."}</p></div><Button variant="secondary" onClick={() => router.push("/onboarding")}>Edit profile</Button></div>
        <div className="mt-7 grid gap-3 border-t border-ink/8 pt-7 sm:grid-cols-3"><ProfileFact icon={<Globe2 />} label="Languages" value={profile.languages.join(" · ") || "Not shared yet"} /><ProfileFact icon={<Sparkles />} label="Skills" value={profile.skills.join(" · ") || "Add skills to your profile"} /><ProfileFact icon={<UserRound />} label="Neighbour moments" value={`${completed} completed`} /></div>
      </section>
      ...
      <HelperDetails />
      {profile.isGuest ? <p className="mt-5 rounded-2xl bg-sun/25 p-4 text-sm leading-6">This temporary identity belongs to this browser. It cannot be recovered after signing out or clearing browser data.</p> : null}
      {error ? <p role="alert" className="mt-4 text-coral">{error}</p> : null}
