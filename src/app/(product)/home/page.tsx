  ...
  return <div className="mx-auto max-w-4xl space-y-7">
    <section>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        <Link href="/ask" className="flex min-h-28 items-center gap-4 rounded-[28px] bg-purple p-5 text-white shadow-lg shadow-purple/15"><Mic className="size-8 shrink-0" /><span><strong className="block text-xl font-black">Ask for help</strong><span className="text-sm text-white/75">Tell KAKI what you need</span></span><ArrowRight className="ml-auto size-5" /></Link>
        <Link href="/discover" className="flex min-h-28 items-center gap-4 rounded-[28px] bg-sun p-5 text-ink"><HandHeart className="size-8 shrink-0" /><span><strong className="block text-xl font-black">Help a neighbour</strong><span className="text-sm text-ink/65">A few minutes can mean a lot</span></span><ArrowRight className="ml-auto size-5" /></Link>
      </div>
    </section>
    ...
    <section><div className="flex items-center justify-between gap-3"><h2 className="text-xl font-black">Someone could use your help</h2><Link href="/discover" className="shrink-0 text-sm font-bold text-purple">See all →</Link></div>
      {available.length ? <div className="mt-4 grid gap-4 sm:grid-cols-2">{available.slice(0, 2).map(mission => <MissionCard key={mission.id} mission={mission} />)}</div> : <div className="paper-card mt-4 rounded-[28px] p-6"><p className="text-sm leading-6 text-muted">No requests waiting right now. Need a hand or want to learn a skill?</p><ButtonLink href="/ask" variant="secondary" className="mt-4">Ask a neighbour</ButtonLink></div>}
    </section>
