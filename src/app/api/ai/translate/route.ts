...
export async function POST(request:Request) {
  try {
    ...
    if(!process.env.OPENAI_API_KEY)return NextResponse.json({error:"Translation is not configured."},{status:503});
    if(!await takeAiQuota())return NextResponse.json({error:"AI limit reached. Try again later."},{status:429});
    ...
    return NextResponse.json({text,language:parsed.data.language,mode:"openai"},{headers:{"Cache-Control":"no-store"}});
  } catch(error) {
    return NextResponse.json({error:"Translation unavailable. Your original text is unchanged."},{status:error instanceof SyntaxError ? 400 : error instanceof Error && error.message==="Payload too large" ? 413 : 502});
  }
}
