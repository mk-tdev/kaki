...
export function Select({ value, onChange, options, label, disabled = false, className = "" }: {
  ...
  return <div ref={wrapper} className={`relative min-w-0 ${className}`} onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setOpen(false); }}>
    ...
    {open && !disabled ? <div ref={list} role="listbox" id={`${id}-options`} aria-label={label} className="absolute inset-x-0 top-full z-40 mt-2 max-h-64 overflow-y-auto overscroll-contain rounded-2xl border border-purple/20 bg-paper p-2 shadow-[0_16px_40px_#211d3530]" onKeyDown={event => {
