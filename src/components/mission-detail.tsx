...
import { ArrowLeft, Check, Flower2, MapPin, Navigation, Send, ShieldCheck, Sparkles } from "lucide-react";
...
export function MissionDetail({ missionId }: { missionId: string }) {
  ...
  return <div className="mx-auto max-w-6xl">
    ...
    <div className="grid items-start gap-6 lg:grid-cols-[1.2fr_.8fr]"><div className="space-y-6">
      <section className="grain overflow-hidden rounded-[36px] bg-ink p-7 text-white sm:p-9"><div className="flex items-center justify-between gap-3"><CategoryIcon category={mission.category} className="size-12" /><span className="rounded-full bg-white/10 px-3 py-2 text-xs font-black">{statusLabel}</span></div><p className="mt-6 text-xs font-black uppercase tracking-[.17em] text-sun">{categoryMeta[mission.category].label}</p><h1 className="mt-3 text-4xl font-black leading-[1.05] tracking-[-.05em] sm:text-5xl">{mission.title}</h1><p className="mt-4 leading-8 text-white/70">{mission.summary}</p><p className="mt-6 text-sm font-bold text-sun">{mission.durationMinutes} minutes · {mission.language}</p><p className="mt-3 flex items-center gap-2 text-sm"><MapPin className="size-4" />{mission.location}</p><p className="mt-4 text-xs text-white/60">Requested: {new Intl.DateTimeFormat("en-SG",{dateStyle:"medium",timeStyle:"short",timeZone:"Asia/Singapore"}).format(new Date(mission.scheduledAt))} SGT · Confirm in chat</p></section>
      {mission.status==="completed"?<BloomCelebration shared={shared}/>:null}
      {connected?<JourneyMap mission={mission} presence={presence}/>:null}
      <section className="paper-card rounded-[32px] p-7"><h2 className="flex items-center gap-2 text-2xl font-black tracking-tight"><Sparkles className="size-5 text-purple" />Your Kaki guide</h2><ol className="mt-6 space-y-5">{mission.guide.map((step,index)=><li key={index} className="flex items-start gap-3 text-sm leading-7 text-muted"><span className="grid size-8 shrink-0 place-items-center rounded-full bg-purple/10 font-black text-purple">{index+1}</span>{step}</li>)}</ol><p className="mt-6 flex gap-3 rounded-2xl bg-mint/60 p-4 text-xs leading-6 text-[#17654d]"><ShieldCheck className="mt-1 size-4 shrink-0" />Meet in an agreed public place. Don’t share passwords, payment details or a home address. For concerns, approach the event organiser in person.</p></section>
      {connected?<section className="paper-card rounded-[32px] p-6" id="mission-chat"><h2 className="text-2xl font-black tracking-tight">Say hello 👋</h2><p className="mt-1 text-xs text-muted">Private to participants and organisers · updates automatically</p><div className="mt-5 max-h-80 space-y-3 overflow-y-auto" aria-live="polite" aria-relevant="additions">{messages.length?messages.map(item=><div key={item.id} className={`max-w-[90%] rounded-2xl p-4 text-sm leading-6 ${item.sender.id===profile.id?"ml-auto bg-purple text-white":"bg-ink/5"}`}><p className="whitespace-pre-wrap break-words">{item.body}</p><p className="mt-1 text-[10px] opacity-60">{item.sender.id===profile.id?"You":item.sender.name}</p></div>):<p className="py-6 text-center text-sm text-muted">Confirm your meeting point and say what you can help with.</p>}</div><form className="mt-5 flex gap-2" onSubmit={e=>{e.preventDefault();void send();}}><input ref={messageInput} aria-label="Message your neighbour" maxLength={1000} value={message} onChange={e=>setMessage(e.target.value)} placeholder="Say hello, Kaki…" className="min-w-0 flex-1 rounded-full border border-ink/15 bg-white px-4 text-sm"/><Button type="submit" aria-label="Send message" disabled={busy||!message.trim()} className="size-12 p-0"><Send className="size-4"/></Button></form></section>:null}
    </div><aside className="space-y-6">
      <section className="paper-card rounded-[32px] p-6"><p className="text-xs font-black uppercase tracking-[.15em] text-purple">Your neighbours</p><div className="mt-5 space-y-5">{[mission.requester,...(mission.helper?[mission.helper]:[])].map(person=><div key={person.id} className="flex items-center gap-3"><Avatar profile={person} size="lg"/><div><p className="font-black">{person.name}{person.id===profile.id?" (you)":""}</p><p className="mt-1 text-xs text-muted">{person.id===mission.requester.id?"Asking for help":"Stepping forward"} · {person.verified?"Verified":"Not identity-verified"}</p></div>{presence.some(p=>p.user_id===person.id&&p.arrived_at)?<Check aria-label="Checked in" className="ml-auto size-5 text-kaki-green"/>:null}</div>)}</div></section>
      <section className="paper-card rounded-[32px] p-6"><h2 className="text-2xl font-black tracking-tight">{mission.status==="open"?isRequester?"Your request is live.":"Be someone’s Kaki.":"One step at a time."}</h2><p className="mt-3 text-sm leading-7 text-muted">{mission.status==="open"?isRequester?"Keep this page open. It updates when someone steps forward.":"Only accept if you feel comfortable with the task. Confirm the details together.":mission.status==="matched"?"Say hello, agree where to meet, then each tap ‘I’m here’. The helper can start once both have checked in.":mission.status==="in_progress"?"Follow the guide together. Finish when you’re both happy with the moment.":mission.status==="flagged"?"This request is held out of the community board. Approach an organiser; this is not an emergency response service.":"Thank you for showing up for your neighbourhood."}</p>
      ...
      {connected?<MatchReveal missionId={missionId} onIcebreaker={text=>{setMessage(text);messageInput.current?.focus();messageInput.current?.scrollIntoView({block:"center",behavior:"smooth"});}}/>:null}
    </aside></div>
  </div>;
}
