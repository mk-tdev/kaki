...
export function MissionDetail({ missionId }: { missionId: string }) {
  const { profile, missions, acceptMission, cancelMission, startMission, completeMission, syncError } = useMissions();
  ...
  const canLeave=(isRequester || isHelper) && ["draft","open","flagged","matched","in_progress"].includes(mission.status);
  const statusLabel={draft:"Draft",open:"Looking for a Kaki",matched:"Kaki found",in_progress:"Happening now",completed:"A Bloom is born",cancelled:"Plans changed",flagged:"Needs organiser review"}[mission.status];
  ...
  return <div className="mx-auto w-full min-w-0 max-w-6xl [overflow-wrap:anywhere]">
    ...
    {syncError||coordinationError?<p role="status" className="mb-4 rounded-2xl bg-sun/30 p-4 text-sm">{syncError||coordinationError}</p>:null}
      <section className="grain overflow-hidden rounded-2xl bg-ink p-5 text-white sm:p-7"><div className="flex items-center justify-between gap-3"><CategoryIcon category={mission.category} className="size-12" /><span className="rounded-full bg-white/10 px-3 py-2 text-xs font-black">{statusLabel}</span></div><p className="mt-4 text-xs font-black uppercase tracking-[.17em] text-sun">{categoryMeta[mission.category].label}</p><h1 className="mt-3 text-2xl font-black leading-[1.05] tracking-[-.05em] sm:text-4xl">{mission.title}</h1><details className="mt-3 text-sm text-white/70"><summary className="cursor-pointer font-bold">About this request</summary><p className="mt-2 leading-6">{mission.summary}</p></details><p className="mt-4 text-sm font-bold text-sun">{mission.durationMinutes} minutes · {mission.language}</p><p className="mt-3 flex items-center gap-2 text-sm"><MapPin className="size-4 shrink-0" /><span className="min-w-0">{mission.location}</span></p><p className="mt-4 text-xs text-white/60">Requested: {new Intl.DateTimeFormat("en-SG",{dateStyle:"medium",timeStyle:"short",timeZone:"Asia/Singapore"}).format(new Date(mission.scheduledAt))} SGT · Confirm in chat</p></section>
    <div className="mt-5 grid grid-cols-1 items-start gap-5 lg:grid-cols-[minmax(0,.8fr)_minmax(0,1.2fr)]"><aside className="min-w-0 space-y-4">
      <section className="paper-card rounded-2xl p-4 sm:p-6"><p className="text-xs font-black uppercase tracking-[.15em] text-purple">Your neighbours</p><div className="mt-3 space-y-3">{[mission.requester,...(mission.helper?[mission.helper]:[])].map(person=><div key={person.id} className="flex items-center gap-3"><Avatar profile={person} size="md"/><div className="min-w-0 flex-1"><p className="font-black">{person.name}{person.id===profile.id?" (you)":""}</p><p className="mt-1 text-xs text-muted">{person.id===mission.requester.id?"Asking for help":"Stepping forward"} · {person.verified?"Verified":"Not identity-verified"}</p></div>{presence.some(p=>p.user_id===person.id&&p.arrived_at)?<Check aria-label="Checked in" className="ml-auto size-5 text-kaki-green"/>:null}</div>)}</div></section>
      <section className="paper-card rounded-2xl p-4 sm:p-6"><h2 className="text-xl font-black tracking-tight">{mission.status==="open"?isRequester?"Your request is live.":"Be someone’s Kaki.":mission.status==="cancelled"?"Plans changed. That’s okay.":"Your next step"}</h2><p className="mt-3 text-sm leading-7 text-muted">{mission.status==="open"?isRequester?"We’ll update this page when a neighbour offers to help.":"Can you lend a hand? Say hello and agree the details together.":mission.status==="matched"?"Say hello in chat. Confirm where to meet, then tap ‘I’m here’.":mission.status==="in_progress"?"Help, learn and share. Finish when you’re both happy.":mission.status==="cancelled"?"This request is closed. No need to travel or check in. You can connect again when the time is right.":mission.status==="flagged"?"This request is held out of the community board. Approach an organiser; this is not an emergency response service.":"Thank you for showing up for your neighbourhood."}</p>
        ...
        {connected&&active?<div className="mt-5 space-y-3">{mission.status==="matched"&&!mine?.arrived_at?<><Button variant="secondary" className="w-full" disabled={busy||Boolean(mine?.on_way_at)} onClick={()=>void act(()=>updatePresence("on_way"))}><Navigation className="size-4"/>{mine?.on_way_at?"On my way ✓":"I’m on my way"}</Button><Button className="w-full" disabled={busy} onClick={()=>void act(()=>updatePresence("check_in"))}><MapPin className="size-4"/>I’m here</Button></>:mine?.arrived_at?<p className="rounded-xl bg-mint/60 p-3 text-center text-sm font-bold text-kaki-green">You’re checked in ✓</p>:null}
          <details className="rounded-2xl bg-ink/5 p-3"><summary className="cursor-pointer text-xs font-bold text-muted">Optional: share our story</summary><label className="flex cursor-pointer items-start gap-3 rounded-2xl bg-ink/5 p-4 text-xs leading-6"><Checkbox checked={mine?.consent_to_share??false} disabled={busy} onChange={e=>void act(()=>updatePresence("consent",e.target.checked))}/><span>I agree to share our short story and first names on the public Bloom wall. Optional; both people must agree.</span></label></details>
        ...
        {canLeave?<LeaveRequest matched={Boolean(mission.helper)} busy={busy} onConfirm={()=>act(()=>cancelMission(missionId))}/>:null}
        {mission.status==="cancelled"?<ButtonLink href={isRequester?"/ask":"/discover"} className="mt-4 w-full">{isRequester?"Ask again when ready":"Find another neighbour"}</ButtonLink>:null}
      ...
      {connected&&mission.status!=="cancelled"?<JourneyMap mission={mission} presence={presence}/>:null}
      <details className="paper-card group rounded-2xl p-5"><summary className="flex cursor-pointer list-none items-center gap-2 text-lg font-black tracking-tight [&::-webkit-details-marker]:hidden"><Sparkles className="size-5 text-purple" />Your Kaki guide<ChevronDown className="ml-auto size-5 transition-transform group-open:rotate-180" /></summary><ol className="mt-6 space-y-5">{mission.guide.map((step,index)=><li key={index} className="flex items-start gap-3 text-sm leading-7 text-muted"><span className="grid size-8 shrink-0 place-items-center rounded-full bg-purple/10 font-black text-purple">{index+1}</span>{step}</li>)}</ol><p className="mt-6 flex gap-3 rounded-2xl bg-mint/60 p-4 text-xs leading-6 text-[#17654d]"><ShieldCheck className="mt-1 size-4 shrink-0" />Meet in an agreed public place. Don’t share passwords, payment details or a home address. For concerns, approach the event organiser in person.</p></details>
      {connected&&active?<MatchReveal missionId={missionId} onIcebreaker={text=>{setMessage(text);setChatOpen(true);}}/>:null}
</div></div>
      {connected?<ChatDialog open={chatOpen} onClose={()=>setChatOpen(false)}><section className="paper-card rounded-2xl p-4 sm:p-6" id="mission-chat"><h2 className="text-2xl font-black tracking-tight">Say hello 👋</h2><p className="mt-1 text-xs text-muted">Private to participants and organisers · updates automatically</p><div className="mt-5 max-h-80 space-y-3 overflow-y-auto" aria-live="polite" aria-relevant="additions">{messages.length?messages.map(item=><div key={item.id} className={`max-w-[90%] rounded-2xl p-4 text-sm leading-6 ${item.sender.id===profile.id?"ml-auto bg-purple text-white":"bg-ink/5"}`}><p className="whitespace-pre-wrap break-words">{item.body}</p><details className="mt-2"><summary className="cursor-pointer text-xs font-bold">Translate message</summary><TranslateText text={item.body}/></details><p className="mt-1 text-[10px] opacity-60">{item.sender.id===profile.id?"You":item.sender.name}</p></div>):<p className="py-6 text-center text-sm text-muted">Confirm your meeting point and say what you can help with.</p>}</div><form className="mt-5 flex gap-2" onSubmit={e=>{e.preventDefault();void send();}}><input ref={messageInput} aria-label="Message your neighbour" maxLength={1000} value={message} onChange={e=>setMessage(e.target.value)} placeholder="Say hello, Kaki…" className="min-w-0 flex-1 rounded-full border border-ink/15 bg-white px-4 text-sm"/><Button type="submit" aria-label="Send message" disabled={busy||!message.trim()} className="size-12 p-0"><Send className="size-4"/></Button></form>{error?<p role="alert" className="mt-3 text-sm text-coral">{error}</p>:null}</section></ChatDialog>:null}
